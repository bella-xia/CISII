...
if __name__ == "__main__":
    ...
    try:
        for img_dir in tqdm(sorted_img_dir[1250:1400]):
            timestamp = float(img_dir.split("_")[-1].split(".jpg")[0])
            img_path = os.path.join(IMG_PATH, img_dir)
            img = cv2.imread(img_path)
            cv_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            numeric_data, mask, flag, ka_t, kv_t = processor.serialized_processing(
                cv_img
            )
            visual.add_data(cv_img, mask, numeric_data + [ka_t, kv_t], flag=flag)
